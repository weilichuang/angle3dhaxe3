...
class SkinnedMeshLecture extends BasicLecture
{
	static function main() 
	{
		flash.Lib.current.addChild(new SkinnedMeshLecture());
	}
	...
	private function _loadComplete(loader:FilesLoader):Void
	{
		...
		var node:Node = createNinja(0);
		node.setTranslationXYZ(0, 0, 0);
		...
		camera.location.setTo(Math.cos(0) * 20, 20, Math.sin(0) * 20);
