...
class SgslParser2
{
	...
	private function parseStatement(parent:SgslNode,isInsideFunction:Bool):Void
	{
		...
		else if (!isInsideFunction && type == TokenType2.REGISTERTYPE)
		{
			parent.addChild(parseShaderVar());
			return;
		}
		else if (type == TokenType2.PREPROCESOR)
		{
			parent.addChild(parsePredefine(isInsideFunction));
			return;
		}
		else if (type == TokenType2.DATATYPE)
		{
		...
		else if(getToken().type == TokenType2.WORD)
		{
	...
	{
		var bn:FunctionCallNode = new FunctionCallNode(accept(TokenType2.WORD).text);
	...
	{
		var dataType:String = accept(TokenType2.DATATYPE).text;
		var name:String = accept(TokenType2.WORD).text;
	...
	private function parseAtomExpression():LeafNode
	{
		...
		if (token.type == TokenType2.WORD)
		{
			...
			{
				ret = new AtomNode(accept(TokenType2.WORD).text);
			}
		}
		else if (token.type == TokenType2.NUMBER)
		{
			ret = new ConstantNode(Std.parseFloat(accept(TokenType2.NUMBER).text));
		}
		else if (token.text == "(")
		{
	...
	{
		var bn:AtomNode = new AtomNode(accept(TokenType2.WORD).text);
	...
	private function parseMask(parent:LeafNode):Void
	{
		if (getToken().text == ".")
		{
			...
			parent.mask = accept(TokenType2.WORD).text;
	...
	private function parsePredefine(isInsideFunction:Bool):LeafNode
	{
		...
		while (getToken().type == TokenType2.PREPROCESOR && 
	...
	private function parseSubPredefine(isInsideFunction:Bool):PredefineSubNode
	{
		...
		accept(TokenType2.PREPROCESOR); //SKIP '#ifdef'
		...
		if (subNode.hasParam())
		{
			...
			//至少有一个参数
			subNode.addKeyword(accept(TokenType2.WORD).text);
			...
			if (getToken().text != ")")
			{
				while (getToken().text != ")")
				{
					if (getToken().text == "&&")
					{
						subNode.addKeyword(accept(TokenType2.OPERATOR).text);
					}
					else if (getToken().text == "||")
					{
						subNode.addKeyword(accept(TokenType2.OPERATOR).text);
	...
	{
		if (getToken().type != TokenType2.REGISTERTYPE)
		{
		...
		var registerType:String = accept(TokenType2.REGISTERTYPE).text;
		...
		if (getToken().text == "[")
		{
			...
			if (getToken().type != TokenType2.NUMBER)
			{
				error(getToken(), "Array index only support const value, but is :" + getToken().text);
			}
			...
			arraySize = Std.parseInt(accept(TokenType2.NUMBER).text);
		...
		{
			acceptText("(");
			bindName = accept(TokenType2.WORD).text;
			acceptText(")");
		}
	...
	private function error(t:Token, message:String) : Void
	{
