package angle3d.types;

private typedef InnerData = #if js Float32Expand #else Array<angle3d.types.Float32> #end
...
abstract FloatBuffer(InnerData) {
	...
	public inline function push( v : angle3d.types.Float32 ) {
		this.push(v);
	}
	...
	@:arrayAccess inline function arrayRead(key:Int) : angle3d.types.Float32 {
		return this[key];
	}

	@:arrayAccess inline function arrayWrite(key:Int, value : angle3d.types.Float32) : angle3d.types.Float32 {
		return this[key] = value;
	}

	@:from
	static public inline function fromIntArray(arr:Array<Int>) {
		var floatArray = new FloatBuffer(arr.length);
		for (i in 0...arr.length) {
			floatArray.push(arr[i]);
		}
		return floatArray;
	}

	@:from
	static public inline function fromFloatArray(arr:Array<Float>) {
		var floatArray = new FloatBuffer(arr.length);
		for (i in 0...arr.length) {
			floatArray.push(arr[i]);
		}
